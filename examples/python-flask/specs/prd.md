# PRD: Meeting Notes (Flask reference slice)
...
| D5 | Domain rules live in `app/notes.py` and import no Flask. | Rules tested through a test client need an app context and run an order of magnitude slower. | Rule tests get slow, then get skipped, then the rules drift. |
...
| `published_at` | ISO 8601 or null | Set once, by D1. Null while a draft. |
