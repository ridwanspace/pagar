# PRD: Meeting Notes (Spring Boot reference slice)
...
| D5 | Domain rules live in `NoteService` and are tested with plain JUnit, no `@SpringBootTest`. | Loading an application context per rule test turns milliseconds into seconds. | Rule tests get slow, then get skipped, then the rules drift. |
...
| D7 | `Note` is a record, so publishing returns a new value rather than mutating. | A mutable entity handed to two callers lets one change what the other is still reading. | Aliasing bugs that only appear under concurrency. |
...
| `published_at` | ISO 8601 or null | Set once, by D1. Null while a draft. `publishedAt` is an `Instant`, serialized as ISO 8601 by the Jackson setting in `application.properties`. |
