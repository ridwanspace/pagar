# PRD: Meeting Notes (Go reference slice)
...
| D5 | Domain rules live in `internal/notes/notes.go` and import no `net/http`. | Rules tested through an HTTP recorder need a request, a response writer, and JSON round-tripping. | Rule tests get slow and indirect, and the rules drift. |
...
| D7 | The store is guarded by a mutex, and `Publish` stamps under that lock. | `net/http` serves every request on its own goroutine, so the map is shared from the first request onward. | A data race that `go test -race` catches, or two goroutines both stamping a timestamp. |
...
| `published_at` | ISO 8601 or null | Set once, by D1. Null while a draft. `published_at` is `*time.Time` so it can serialize as JSON `null` while a draft. |
